...
-- Defines source repositories created by merging these into new target
-- repository. The target repositories differ from normal ones in
-- the Catalogs table by having NULL EXTURL and EXTHOST value.
create table MergedCatalogs(SOURCECATALOGID INT NOT NULL,
                            TARGETCATALOGID INT NOT NULL,
                            LASTMERGE       TIMESTAMP,
                            PRIMARY KEY (SOURCECATALOGID, TARGETCATALOGID)
                         -- FOREIGN KEY (SOURCECATALOGID) REFERENCES Catalogs(ID)
                         -- FOREIGN KEY (TARGETCATALOGID) REFERENCES Catalogs(ID)
                           );
...
create table Catalogs(ID          INT AUTO_INCREMENT UNIQUE KEY,
                      ...
                      EXTHOST     VARCHAR(300),  
                      EXTURL      VARCHAR(300),  -- where to mirror from. If NULL, then it's a merged repository (see MergedCatalogs)
...
create table ProductCatalogs(PRODUCTDATAID integer NOT NULL,
                          ...
                          -- FOREIGN KEY (PRODUCTDATAID) REFERENCES Products(PRODUCTDATAID)
